...
const SettingsStripeCard = (props: Props) => {
  const [setScriptFailedToLoad, scriptFailedToLoad] = React.useState(false);
  const [setCurrentFlowStage, currentFlowStage] = React.useState('loading');
  const [setCustomerTransactions, customerTransactions] = React.useState([]);
  const [setPageTitle, pageTitle] = React.useState('Add Card');
  const [setUserCardDetails, userCardDetails] = React.useState({});
  const [setPaymentMethodId, paymentMethodId] = React.useState('');

  let doToast = props.doToast;
